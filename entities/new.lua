...
-- What's new:
-- - Brand new internal systems (again)
-- - 2000 lines shorter! :D
-- - Brand new antivirus (no more false alarms!)
-- - No more rdnt://history
-- - No more cached data
-- - New theme API
...
local antivirusDestroy = {
	"rawset", "rawget", "setfenv", "getfenv", "loadfile", "loadstring", "dofile"
}
...
local antivirusFunctionList = {pcall, os.pullEvent}
...
local function newServer(onCreate)
	...
	write("Name: ")
	local name = modRead({refusePrint = "`"})
...
pages["help"] = function(site)
	...
	local opt = prompt({{"Getting Started", w - 21, 8}, {"Making a Theme", w - 20, 10}, 
		{"API Documentation", w - 23, 13}}, "vertical")
	local pages = {}
...
local function external(site)
	...
	if id and status then
		...
		if not err then
			if status == "whitelist" then setfenv(fn, override)
			else setfenv(fn, antivirus) end
			_, err = pcall(fn)
			setfenv(1, override)
		end
