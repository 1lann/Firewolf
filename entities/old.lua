...
local backupYield = coroutine.yield
...
local function resetFilesystem()
	...
	if not(ffs.exists(serverSoftwareLocation)) then
		if not getDropbox(serverURL, serverSoftwareLocation) then
			term.clear()
			term.setCursorPos(1,1)
			fprint("Error: Could not download server software")
			ferror()
		end
	end
...
local function updateClient()
	...
	if getDropbox(firefoxURL, updateLocation) then
...
local function webpageOverspeed(overspeed)
	...
	for i = 1, 5 do
		...
		fprint(" ")
		centerPrint("To prevent Rednet spamming and being throttled")
...
local function loadWebpage(site)
	...
	local function runSite(cacheLoc)
		...
		local _, websiteError = fpcall(fparallel.waitForAll(ctrlControl, function()
			fshell.run(cacheLoc) 
		end))
		if websiteError then fprint(websiteError) end
...
local function renderWebpage(site)
	...
	if fos.clock() - loadingRateClock > 5 then
		websiteLoadingRate = 0
		loadingRateClock = fos.clock()
	elseif websiteLoadingRate >= 4 then
		overspeed = true
	end
...
local function startup()
	...
	if debugging then
		...
		rawset(coroutine, "yield", function(a)
