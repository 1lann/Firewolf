...
--  Handshake
	...
	function Handshake.generateInitiatorData()
		...
		Handshake.secret = math.random(10,999999)
		return {
			type = "initiate",
...
local serializedData = textutils.serialize(Handshake.generateInitiatorData())
Modem.send(Handshake.channel, Handshake.packetHeader..serializedData)
...
while true do
	...
	if e == "modem_message" then
		if msg:match(Handshake.packetMatch) then
			...
			secret = Handshake.generateResponseData(unserialized)
			distance = p5
			print("key: ",secret)
			break
		end
...
print("Sending...")
print(connection:sendMessage(f:read("*a")))
f:close()
print("message sent")