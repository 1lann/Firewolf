...
--  
--  Firewolf 1.58 Fallback
--  Made by GravityScore and 1lann
--  

-- Copied and pasted window api
local rootTerm = {}
...
function window.create( _, nX, nY, nWidth, nHeight, bStartVisible )
	if type( nX ) ~= "number" or
	   type( nY ) ~= "number" or
...
local allowUnencryptedConnections = false
...
local determineLanguageForContents = function(contents)
	return "lua"
end
...
local fetchExternal = function(url, connection)
	...
	local contents = connection.fetchPage(page)
	connection.close()
	if contents then
		if type(contents) ~= "string" then
			...
			if err then
				...
				local language = determineLanguageForContents(contents)
				return languages[language]["run"](func)
			end
		end
	else
...
languages["lua"]["run"] = function(func, ...)
	local args = {...}
...
local originalTerminal = term.native
